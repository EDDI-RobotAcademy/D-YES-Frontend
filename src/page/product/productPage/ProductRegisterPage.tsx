...
const ProductRegisterPage = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const { productName, productDescription, cultivationMethod } = target.elements
    
    const optionObjects = useOptions.map(option => ({
  ...
  return (
    <Container maxWidth="md" sx={{ marginTop: '2em' }}>
      ...
      <Box display="flex" flexDirection="column" gap={2} p={2}>
        <h1>상품 등록</h1>
        <ToggleComponent label='기본정보'>
          <Box display="flex" flexDirection="column" gap={2}>
            <div className='text-field-container'>
              <div className="text-field-label">상품명</div>
        ...
        <ToggleComponent label='이미지'>
          메인 이미지 등록, 상세 이미지 등록 추가
        </ToggleComponent>
        <ToggleComponent label='옵션정보'>
          <Box display="flex" flexDirection="column" gap={2}>
            <OptionTable
        ...
        <ToggleComponent label='상세정보'>
          <Box display="flex" flexDirection="row" alignItems="center" gap={2}>
            <div  className="text-field-label">상세정보</div>
            <TextField name="productDescription" className="text-field-input text-field" 
              multiline
              minRows={10}
              maxRows={20}/>
          </Box>
